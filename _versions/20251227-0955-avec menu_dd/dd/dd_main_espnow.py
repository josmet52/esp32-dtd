"""
dd_main_espnow.py v6.0.0 - Programme principal DD avec ESP-NOW
Module: WROVER-T7 uniquement

Changements v5.1.0:
    - Ajout traitement commande MODE: pour synchronisation auto avec TA
    - Commandes supportées: MODE:ESPNOW, MODE:433MHZ
    - Changement automatique de mode + reboot

Changements v5.0.0:
    - Remplacement UART RS-485 par ESP-NOW (sans fil)
    - RSSI disponible pour chaque communication
    - Suppression des pins UART (18/23 libérés)
    - Protocol identique: POLL:XX → ACK:XX:Y
    - MAC du TA configuré via NVS (clé: "ta_mac")

Configuration requise:
    - Stocker MAC du TA en NVS: nvs_utils.set_str("DTD", "ta_mac", "AA:BB:CC:DD:EE:FF")
    - Pas de module radio externe nécessaire
"""
...
class DD_Radio:
    ...
    def __init__(self, ta_mac_str):
        # Activer WiFi (nécessaire pour ESP-NOW)
    ...
    def get_rssi(self):
        ...
        try:
            ...
            # WiFi pas connecté à un réseau (normal pour ESP-NOW)
            return None
    ...
    def recv(self, timeout_ms=0):
        ...
        if msg:
            # Le RSSI n'est pas disponible de manière fiable en ESP-NOW
...
def handle_poll_sequential(line):
    ...
    if req_id == DD_ID:
        # Marquer activité et redémarrer cycle LED
...
def handle_mode_command(line):
    ...
    Cette fonction permet au TA de synchroniser automatiquement
    tous les DD lors d'un changement de mode radio.
    """
    try:
        # Extraire le mode après "MODE:"
        ...
        if mode_str == "ESPNOW":
            ...
            try:
                from dd_nvs_config import DDNVSConfig, RADIO_MODE_ESPNOW
            except:
                # Fallback si dd_nvs_config pas disponible
                print("[MODE] ⚠ dd_nvs_config non disponible")
                return
            
            print("[MODE] → Changement vers ESP-NOW")
            
            if DDNVSConfig.set_radio_mode(RADIO_MODE_ESPNOW):
            ...
            try:
                from dd_nvs_config import DDNVSConfig, RADIO_MODE_433
            except:
                print("[MODE] ⚠ dd_nvs_config non disponible")
                return
            ...
            if DDNVSConfig.set_radio_mode(RADIO_MODE_433):
                print("[MODE] ✓ Mode enregistré en NVS")
                print("[MODE] Reboot dans 500ms...")
                time.sleep_ms(500)
                machine.reset()
...
def main():
    ...
    print("-" * 70)
    print("DTD v5.1.0 ESP-NOW | ID={:02d} | Module: WROVER-T7".format(DD_ID))
