...
def read_dd_id():
    ...
    # Lecture des pins (0 si strap fermé, 1 si ouvert)
...
#   0 = mode normal (démarrage dd_main.py)
#   1 = mode OTA (démarrage ota_mode.py)
OTA_FLAG = nvs_utils.get_i32("DTD", "ota_mode", default=0)

if OTA_FLAG == 1:
    # ============================================================
    # MODE OTA - Mise à jour sans fil
    # ============================================================
    print("DD{:02d}".format(DD_ID))
    print(">>> OTA MODE (from NVS flag) <<<")
    
    # Garbage collect avant OTA pour maximiser la mémoire disponible
    import gc
    gc.collect()
    
    # Lancer OTA directement (mémoire propre, pas de fragmentation)
    try:
        from ota_mode import enter_ota_mode
        
        # IMPORTANT: Le flag sera effacé par ota_mode.py à la fin
        # Ceci garantit que le flag n'est effacé que si OTA se termine normalement
        ...
        # Effacer le flag pour éviter une boucle de reboot
...
# Le programme principal (dd_main.py) démarre automatiquement
# grâce à AUTO_START = True
