"""
Project: DTD - ta_app.py v3.3.1
Mode portrait uniquement - Surveillance détecteurs

Changements v3.3.1:
- Blocage rafraîchissement UI quand menu affiché
- Vérification is_menu_visible() avant updates UI

Changements v3.3.0:
- Support chargement dynamique module radio
- Initialisation menu UI dans ButtonManager
"""
...
class TaApp:
    def __init__(self, radio_module=None, tft=None, ui=None, radio=None, button_manager=None):
        ...
        if radio:
            # Instance radio passée directement
            self.radio = radio
        elif radio_module:
            # Module radio passé par ta_main
            # Déterminer la classe Radio du module
            if hasattr(radio_module, 'RadioESPNow'):
                ...
                # Fallback
                from ta_radio_espnow_sans_wifi import RadioESPNow as RadioClass
            
            self.radio = RadioClass(config.RADIO, logger, ui=self.ui)
        else:
            # Fallback si rien n'est passé
            from ta_radio_espnow_sans_wifi import RadioESPNow as RadioClass
            self.radio = RadioClass(config.RADIO, logger, ui=self.ui)
