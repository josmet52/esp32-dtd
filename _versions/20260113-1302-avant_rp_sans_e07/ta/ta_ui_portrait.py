"""
Project: DTD - ta_ui_portrait.py v1.6.7
Interface utilisateur en mode PORTRAIT pour LilyGO T-Display S3

NOUVEAUTÉ v1.6.7:
- RSSI affiché uniquement si disponible (WiFi connecté)
- Pas d'erreur si RSSI non disponible
- Zone version adaptative selon disponibilité RSSI
...
try:
    ...
    print("[WARN] tft_config ou fonts non trouves - mode simulation")
    tft_config = None
    font_general = None
    font_title = None
    font_small = None
...
class UIPortrait:
    ...
    def __init__(self, tft=None):
        ...
        # Initialisation TFT
        if not self.tft and tft_config:
            try:
                self.tft = tft_config.config(self.rotation)
                logger.info("TFT initialise en mode portrait", "ui_portrait")
            except Exception as e:
                logger.error("Erreur init TFT: {}".format(e), "ui_portrait")
        
        # Effacer l'écran
        if self.tft:
            self.tft.fill(tft_config.BLACK)
            self._draw_title()
            self._draw_version()
    ...
    def _draw_title(self):
        ...
        try:
            ...
            self.tft.fill_rect(0, self.y_title, self.width,
                             self.zone_title_height, tft_config.BLUE)
            ...
            if font_title:
                ...
                self.tft.text(font_title, app_name, text_x, text_y,
                            tft_config.WHITE, tft_config.BLUE)
    ...
    def _draw_dd_line(self, index, state):
        ...
        try:
            ...
            self.tft.fill_rect(0, y_pos, self.width,
                             self.zone_dd_line_height, tft_config.BLACK)
            ...
            if state is True:
                indicator_color = tft_config.GREEN  # Détection présente
                state_text = "ON"
                text_color = tft_config.BLACK
            elif state is False:
                indicator_color = tft_config.RED    # Pas de détection (mais DD répond)
                state_text = "OFF"
                text_color = tft_config.YELLOW
            else:
                indicator_color = tft_config.color565(160, 170, 200)  # Gris CLAIR BRG
                state_text = "no signal"
                text_color = tft_config.WHITE  # Texte BLANC sur gris clair
            ...
            if font_title:
                ...
                self.tft.text(font_title, dd_text, text_x, text_y + decalage,
                            tft_config.WHITE, tft_config.BLACK)
                ...
                if font_title:
                    ...
                    self.tft.text(font_general, state_text, state_text_x, state_text_y,
                ...
                if state is not None and dd_id in self.dd_rssi:
                    ...
                    if font_general:
                        ...
                        self.tft.text(font_general, rssi_text, rssi_x, rssi_y,
                                    tft_config.WHITE, indicator_color)
    ...
    def _draw_version(self):
        """
        Dessine la zone version sur 2 ou 3 lignes selon disponibilité RSSI :
        Ligne 1: Version (ex: "v2.5.0")
        Ligne 2: Batterie (ex: "3.7V 76%")
        Ligne 3: RSSI moyen (ex: "RSSI: -52dBm") - SI DISPONIBLE
        ...
        try:
            ...
            self.tft.fill_rect(0, self.y_version, self.width,
                             self.zone_version_height, tft_config.BLACK)
            ...
            if self.battery_voltage is not None:
                ...
                if font_general:
                    self.tft.text(font_general, text2, text2_x, text2_y,
                                tft_config.CYAN, tft_config.BLACK)
            
            # Ligne 3: RSSI MOYEN (font_general = 16x16) - UNIQUEMENT SI DISPONIBLE
            if self.avg_rssi is not None:
                ...
                if font_general:
                    self.tft.text(font_general, text3, text3_x, text3_y,
                                tft_config.YELLOW, tft_config.BLACK)
    ...
    def _draw_heartbeat(self):
        ...
        try:
            color = tft_config.CYAN if self.heartbeat_on else tft_config.BLACK
