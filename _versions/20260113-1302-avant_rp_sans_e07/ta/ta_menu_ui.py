...
- Menu simplifié: ESP-NOW, Radio 433, OTA, Reboot
...
# Entrees du menu (indices modifies)
MENU_ESP_NORMAL = 0
MENU_433MHZ = 1
MENU_OTA = 2
MENU_REBOOT = 3
...
class MenuUI:
    ...
    def __init__(self, ui):
        ...
        # Options du menu (mode RSSI supprime)
        self.menu_items = [
            "1. ESP-NOW",
            "2. Radio 433MHz",
            "3. OTA Update",
    ...
    def _switch_to_portrait(self):
        ...
        try:
            ...
            if self.original_rotation is not None:
                ...
                self.tft.rotation(0)  # Defaut portrait
    ...
    def show_menu(self, selected_index=0):
        ...
        Args:
            selected_index (int): Index de l'option sélectionnée (0-3)
        """
        ...
        try:
            ...
            title_y = 15  # Reduit de 20 a 15
            self.tft.text(self.font_title, title, title_x, title_y, 
                         tft_config.WHITE, bg_color)
            
            # Ligne sous le titre
            line_y = title_y + 30  # Reduit de 35 a 30
            self.tft.hline(20, line_y, self.width - 40, tft_config.WHITE)
            
            # Position de depart des items
            items_start_y = line_y + 15  # Reduit de 20 a 15
            item_spacing = 26  # Reduit de 30 a 26
            
            # Afficher chaque option
            for i, item in enumerate(self.menu_items):
                y_pos = items_start_y + (i * item_spacing)
                
                # Couleurs selon selection
                if i == selected_index:
                    # Option selectionnee : fond vert, texte blanc
                    bg = tft_config.color565(0, 100, 0)
                    fg = tft_config.WHITE
                    
                    # Rectangle de selection (hauteur reduite)
                    self.tft.fill_rect(10, y_pos - 3, self.width - 20, 22, bg)
                else:
                    # Option non selectionnee : fond bleu, texte blanc
                    bg = bg_color
                    fg = tft_config.WHITE
            ...
            # Instructions en bas (position ajustee)
            help_y = self.height - 35  # Reduit de 40 a 35
            self.tft.text(self.font_item, "Court: Suivant  |  Long: Valider", 20, help_y, 
                         tft_config.YELLOW, bg_color)
    ...
    def show_confirmation(self, item_index):
        ...
        try:
            # Deja en paysage normalement, mais on s'assure
            ...
            for i, msg in enumerate(messages):
                if msg:
                    ...
                    self.tft.text(font, msg, msg_x, msg_y, tft_config.WHITE, bg_color)
