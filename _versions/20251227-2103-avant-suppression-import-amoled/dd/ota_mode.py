...
class OTAMode:
        ...
        try:
            ...
            try:
                ...
                if cmd == 'announce_request':
                    ...
                    # Envoyer immediatement l'annonce
                    self.announce_to_ta()
    ...
    def start_ota_server(self):
        ...
        sock.bind(addr)
        sock.listen(1)
        sock.settimeout(30.0)  # 30s timeout pour annonces periodiques
        
        print("[OTA] Ready")
        
        # Compteur pour annonces periodiques
        announce_counter = 0
        
        try:
            while True:
                # Verifier les commandes UDP du TA
                ...
                try:
                    ...
                    if 'timeout' in str(e).lower():
                        # Envoyer annonce UDP periodiquement
                        announce_counter += 1
        ...
        for line in lines:
            ...
            # Methode 2: Content-Disposition
                ...
                if 'filename=' in lower_line:
                    ...
                    if idx != -1:
                        # Extraire apres filename=
                        filename_part = line[idx + 9:].strip()
                        
                        # Verifier si entre guillemets
                ...
                if path == "/upload" or path.startswith("/ota/update"):
                    if content_length > 0:
                        ...
                        if body_start != -1:
                            ...
                            # Methode 1: Extraire le filename depuis les headers HTTP
                            filename = self.extract_filename_from_headers(lines)
                            
                            # Methode 2: Extraire le filename du path si present (ex: /ota/update?file=dd_main.py)
...
<body>
    <div class="container">
        ...
        <div class="upload-zone" id="dropZone" onclick="document.getElementById('fileInput').click()">
            ...
            <div style="color: #666;">ou cliquez pour selectionner</div>
    ...
    def handle_upload(self, client, body, content_type, filename_hint=None):
        ...
        try:
            ...
            # Si pas multipart ou parsing echoue, traiter comme raw body
            if file_content is None:
                print("[OTA] Format: raw body")
                file_content = body
                
                # Si pas de filename fourni, utiliser un nom par defaut
                if not filename:
    ...
    def handle_reboot(self, client):
        ...
        try:
            ...
            # Attendre que la reponse parte
            time.sleep_ms(200)
            
            # Effacer le flag OTA avant de redemarrer
            if nvs_utils:
...
def enter_ota_mode(dd_id):
    ...
    # Nettoyage memoire agressif AVANT de creer l'objet OTA
    gc.collect()
