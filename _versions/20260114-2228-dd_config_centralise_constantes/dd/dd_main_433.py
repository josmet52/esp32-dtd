...
v1.0.2 : 14.01.2026 - Import constantes depuis dd_config
"""

from machine import Pin, ADC, UART
import time
import machine
import dd_config as cfg
...
def read_dd_id():
    ...
    try:
        p0 = Pin(cfg.BIT0_PIN, Pin.IN, Pin.PULL_UP)
        p1 = Pin(cfg.BIT1_PIN, Pin.IN, Pin.PULL_UP)
        p2 = Pin(cfg.BIT2_PIN, Pin.IN, Pin.PULL_UP)
...
# VARIABLES GLOBALES
# ================================================================
STATUS_PIN = Pin(cfg.DD_STATUS_PIN, Pin.IN, Pin.PULL_UP)

# Batterie
BAT_ADC = ADC(Pin(cfg.BATTERY_ADC_PIN))
try:
...
USB_POWER = Pin(cfg.USB_POWER_PIN, Pin.IN, Pin.PULL_DOWN)
LED_INTERNAL = Pin(cfg.LED_INTERNAL_PIN, Pin.OUT)
...
def init_uart():
    ...
    try:
        uart = UART(
            ...
            timeout=cfg.UART_TIMEOUT_MS,
            rxbuf=512
        )
        
        print("[RADIO] UART{} initialisé: {}baud TX={} RX={}".format(
            cfg.UART_INDEX, cfg.UART_BAUD, cfg.UART_TX_PIN, cfg.UART_RX_PIN))
...
def measure_state():
    samples = 0
    for _ in range(cfg.DEBOUNCE_SAMPLES):
        if STATUS_PIN.value() == 0:
            samples += 1
        time.sleep_us(cfg.DEBOUNCE_DELAY_US)
    return 1 if samples >= (cfg.DEBOUNCE_SAMPLES // 2 + 1) else 0
...
def read_battery_voltage():
    try:
        ...
        voltage_bat = voltage_adc * cfg.BAT_VOLTAGE_DIVIDER * battery_correction_factor
        return voltage_bat
    except:
        return None
...
def update_battery_leds(voltage):
    ...
    if not usb_present:
        ...
        if voltage >= cfg.UBAT_MID:
            LED_GREEN.value(1)
            LED_RED.value(0)
        elif voltage >= cfg.UBAT_LOW:
            LED_GREEN.value(1)
            LED_RED.value(1)
        else:
            LED_GREEN.value(0)
        ...
        if voltage > cfg.UBAT_HIGH:
            blink_green = False
            blink_red = False
            blink_both = False
            LED_GREEN.value(1)
            LED_RED.value(0)
        elif voltage >= cfg.UBAT_MID:
            blink_green = True
            blink_red = False
            blink_both = False
            LED_RED.value(0)
...
def update_blinking_leds():
    ...
    now = ticks_ms()
    if ticks_diff(now, last_bat_blink_ms) < cfg.BAT_LED_BLINK_MS:
        return
...
def update_activity_led():
    ...
    if last_poll_time_ms > 0 and ticks_diff(now, last_poll_time_ms) < cfg.ACTIVITY_LED_TIMEOUT_MS:
        time_in_cycle = ticks_diff(now, last_activity_led_cycle_ms)
        
        if time_in_cycle >= cfg.ACTIVITY_LED_PERIOD_MS:
            last_activity_led_cycle_ms = now
            time_in_cycle = 0
        
        should_be_on = (time_in_cycle < cfg.ACTIVITY_LED_ON_MS)
...
def handle_mode_command(line):
    ...
    try:
        ...
        if mode_str == "ESPNOW":
            ...
            if cfg.set_radio_mode(cfg.RADIO_MODE_ESPNOW):
            ...
            if cfg.set_radio_mode(cfg.RADIO_MODE_433):
                print("[MODE] ✓ Mode enregistré en NVS")
                print("[MODE] Reboot dans 500ms...")
                time.sleep_ms(500)
                machine.reset()
...
def main():
    ...
    if cfg.WATCHDOG_ENABLED and WDT is not None:
        wdt = WDT(timeout=cfg.WATCHDOG_MS)
    else:
        wdt = None
    
    print("-" * 70)
    print("DTD v1.0.2 433MHz | ID={:02d} | Module: WROVER-T7".format(DD_ID))
    print("DD_ID = {:02d} | MODE SÉQUENTIEL | COMMUNICATION: 433MHz UART".format(DD_ID))
    print("DEV={} | STAT_PERIOD={} | WATCHDOG={}".format(cfg.DEV_MODE, cfg.STAT_PERIOD, cfg.WATCHDOG_ENABLED))
    print("-" * 70)
    ...
    while True:
        ...
        if loop_count % cfg.STAT_PERIOD == 0:
