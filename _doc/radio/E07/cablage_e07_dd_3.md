...
| **GND** | Masse | GND | - | Commune (2 pins GND sur E07) |
...
| **CSN** | Chip Select | **IO15** | **GPIO15** | CS/SS (boot strap, nécessite pull-up 10kΩ) |
| **GD00** | DIO0/IRQ | **IO36** | **GPIO36** | IRQ principale (input-only OK) |
| **GD02** | DIO2 | **IO39** | **GPIO39** | Optionnel (input-only OK) |

### Connexions optionnelles (contrôle PA/LNA)

| Pin E07 | Fonction | Connexion | Notes |
|---------|----------|-----------|-------|
| **TXEN** | TX Enable | **VCC (3.3V)** via **R 100Ω** | Active PA en TX, ou contrôle GPIO |
| **RXEN** | RX Enable | **VCC (3.3V)** via **R 100Ω** | Active LNA en RX, ou contrôle GPIO |
| **N** | NC / RESET | **Non connecté** | Ou RESET si besoin |

### Notes importantes sur TXEN/RXEN

Le E07 possède un PA/LNA intégré qui peut être contrôlé de 3 façons :

#### Option 1 : Mode automatique (RECOMMANDÉ pour débuter)
- **TXEN → 3.3V** via résistance 100Ω
- **RXEN → 3.3V** via résistance 100Ω
- Le module gère automatiquement TX/RX via le chip SX127x
- **Avantage** : Simple, pas de GPIO supplémentaire
- **Inconvénient** : PA/LNA toujours actifs (consommation légèrement plus élevée)

#### Option 2 : Contrôle logiciel via GPIO (avancé)
Si vous voulez contrôler finement le PA/LNA :
- **TXEN** → GPIO libre (ex: GPIO12)
- **RXEN** → GPIO libre (ex: GPIO14)
- Contrôle manuel selon mode TX/RX
- **Avantage** : Économie d'énergie maximale
- **Inconvénient** : Nécessite 2 GPIO supplémentaires + code

#### Option 3 : PA/LNA désactivés (test simple)
- **TXEN → GND**
- **RXEN → GND**
- **Avantage** : Test rapide sans PA/LNA
- **Inconvénient** : Portée réduite (~50% de la puissance TX)

### Pins non utilisés dans ce câblage

| Pin E07 | Fonction | Statut |
|---------|----------|--------|
| **N** | NC ou RESET | Non connecté (ou RESET si nécessaire) |

**Note :** Le E07 n'a pas de pin RESET dédié exposé. Le reset se fait généralement par power-cycle ou commande SPI.
...
csn = Pin(15, Pin.OUT, value=1)     # CS idle HIGH (+ pull-up 10kΩ requis!)
gd00 = Pin(36, Pin.IN)              # DIO0 - IRQ RX/TX
gd02 = Pin(39, Pin.IN)              # DIO2 - Optionnel

# Option 1 (recommandé pour test) : TXEN/RXEN câblés à VCC via 100Ω
# → Pas de GPIO nécessaire, contrôle automatique

# Option 2 (avancé) : Contrôle GPIO du PA/LNA
# txen = Pin(12, Pin.OUT)  # Si contrôle manuel souhaité
# rxen = Pin(14, Pin.OUT)  # Si contrôle manuel souhaité
```

### Séquence d'initialisation

```python
# Note: Le E07 n'a pas de pin RESET exposé
# Le reset se fait par power-cycle ou commande SPI

# 1. Vérifier version (lecture registre 0x42)
csn.value(0)
...
# 2. Configuration TXEN/RXEN si contrôle GPIO (Option 2)
# txen.value(0)  # PA off
# rxen.value(1)  # LNA on (mode RX par défaut)
```
...
| GND | → | GND | 2 pins GND disponibles |
| MISO | → | GPIO19 | VSPI (libéré) |
| MOSI | → | GPIO23 | VSPI (libéré) |
| SCK | → | GPIO18 | VSPI (libéré) |
| CSN | → | GPIO15 | **+ pull-up 10kΩ requis** |
| GD00 | → | GPIO36 | DIO0 - IRQ principale |
| GD02 | → | GPIO39 | DIO2 - Optionnel |
| TXEN | → | 3.3V | Via résistance 100Ω (mode auto) |
| RXEN | → | 3.3V | Via résistance 100Ω (mode auto) |
| N | → | NC | Non connecté |
...
2. **Résistances 100Ω (x2)** : TXEN → 3.3V et RXEN → 3.3V (mode automatique)
3. **Condensateurs E07** : 10µF + 100nF près du VCC
4. **Condensateurs rail 3.3V** : 470µF + 100nF (recommandé)
5. **Antenne E07** : Adaptée à la fréquence (obligatoire)
